...
NumType LastForSum(const NumType n) {
...
NumType Solve(const Data &data) {
    NumType last_to, last_from = 0;
    last_to = LastForSum(data.to);
    if (data.from > 0) {
        last_from = LastForSum(data.from - 1);
    }
    return (last_to - last_from + 10) % 10;
}
