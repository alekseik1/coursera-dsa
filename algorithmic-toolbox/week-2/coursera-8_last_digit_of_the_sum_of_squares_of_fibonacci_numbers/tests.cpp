...
int FibonacciSumSquaresNaive(int64_t n) {
    if (n <= 1) {
        return n;
    }
...
TEST_CASE("Same as naive") {
    for (NumType to = 0; to <= 40; ++to) {
        REQUIRE(Solve({to}) == FibonacciSumSquaresNaive(to));
    }
}
...
TEST_CASE("Edge cases") {
    REQUIRE(Solve({0}) == 0);
    // 10^14 is checked in stress test below
}

TEST_CASE("Stress test & Overflow & TL") {
